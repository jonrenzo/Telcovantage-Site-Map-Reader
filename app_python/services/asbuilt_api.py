...
def get_sites():
    url = f"{ASBUILT_API_BASE_URL}/asbuilt/sites"
    resp = requests.get(url, headers=_headers(), timeout=API_TIMEOUT)
    resp.raise_for_status()
    return resp.json()

def get_nodes(area_id: int):
    url = f"{ASBUILT_API_BASE_URL}/asbuilt/sites/{area_id}/nodes"
    resp = requests.get(url, headers=_headers(), timeout=API_TIMEOUT)
    resp.raise_for_status()
    return resp.json()

def get_node(node_id: int):
    url = f"{ASBUILT_API_BASE_URL}/asbuilt/node/{node_id}"
    resp = requests.get(url, headers=_headers(), timeout=API_TIMEOUT)
    resp.raise_for_status()
    return resp.json()

def import_data(payload: dict):
    url = f"{ASBUILT_API_BASE_URL}/asbuilt/import"
