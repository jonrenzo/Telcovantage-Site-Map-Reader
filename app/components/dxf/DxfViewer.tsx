    ...
    <div className="flex-1 relative overflow-hidden bg-[#e8edf5]">
        ...
        <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 items-end">
          ...
          {hoveredSpan && (
            <div className="bg-amber-50/95 border border-amber-200 rounded-lg px-3 py-2 text-[11px] text-amber-900 backdrop-blur-sm shadow-sm min-w-[250px]">
              ...
              <div>
                Length:{" "}
                {hoveredSpan.meterValue?.toFixed(2) ??
                  hoveredSpan.total_length.toFixed(2)}{" "}
                meters
              </div>
              {cableStatuses[hoveredSpan.span_id] ===
                "Unrecovered or Partial" && (
                <div className="mt-1 text-[10px]">
                  R: {partialDetails[hoveredSpan.span_id]?.recovered ?? 0} / U:{" "}
                  {partialDetails[hoveredSpan.span_id]?.unrecovered ?? 0} / M:{" "}
                  {partialDetails[hoveredSpan.span_id]?.missing ?? 0}
                </div>
              )}
            </div>
          )}
      ...
      {!loading && !error && (
        <div className="absolute bottom-4 right-4 bg-surface/90 border border-border rounded-lg px-3 py-1.5 text-[10px] font-mono text-muted backdrop-blur-sm">
          {Object.values(segmentsRef.current).flat().length.toLocaleString()}{" "}
          segments across {layers.length} layers
        </div>
      )}
