...
import { getDisplayKind, KIND_COLOR, KIND_LABEL } from "./EquipmentCanvas";
...
export default function EquipmentPanel({
  ...
  // Use display kind (node/amplifier instead of both being "rectangle")
  const allDisplayKinds = useMemo(() => {
    const k = new Set(shapes.map((s) => getDisplayKind(s.kind, s.layer)));
    ...
    <aside className="w-80 bg-surface border-r border-border flex flex-col overflow-hidden flex-shrink-0">
      ...
      <div className="px-5 py-3 border-b border-border flex flex-col gap-2">
        <label className="text-xs font-semibold text-muted uppercase tracking-wider">
          Boundary Layer
          <span className="ml-1 normal-case font-normal text-muted-2">
            (optional)
          </span>
        </label>
        <select
        ...
        <div className="px-5 py-3 border-b border-border">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-1.5">
              <button
                onClick={() => setVisibleKinds(new Set(allDisplayKinds))}
          ...
          <div className="flex flex-wrap gap-1.5">
            {allDisplayKinds.map((dk) => {
              const active = visibleKinds.has(dk);
              const color = KIND_COLOR[dk] ?? "#64748b";
              return (
                <button
                  key={dk}
                  onClick={() => toggleKind(dk)}
                ...
                >
                  ...
                  {KIND_LABEL[dk] ?? dk}
                  <span
                    className={`text-[10px] ${active ? "opacity-70" : "text-muted-2"}`}
                  >
                    {kindCounts[dk] ?? 0}
                  </span>
                </button>
            ...
            <>
              ...
              <div className="max-h-36 overflow-y-auto scrollbar-thin flex flex-col gap-0.5">
                {filteredLayers.map((layer) => {
                  // Show what kind of equipment is on this layer
                  const layerKinds = [
                    ...new Set(
                      shapes
                        .filter((s) => s.layer === layer)
                        .map((s) => getDisplayKind(s.kind, s.layer)),
                    ),
                  ];
                  return (
                    <label
                    ...
                    >
                      ...
                      {/* Color dot(s) for the kind(s) on this layer */}
                      <div className="flex gap-0.5 flex-shrink-0">
                        {layerKinds.map((dk) => (
                          <div
                            key={dk}
                            className="w-2 h-2 rounded-full"
                            style={{ background: KIND_COLOR[dk] ?? "#64748b" }}
                            title={KIND_LABEL[dk] ?? dk}
                          />
                        ))}
                      </div>
        ...
        {filtered.map((shape) => {
          ...
          return (
            <div
              ...
              onClick={() =>
                setSelectedId(
                  shape.shape_id === selectedId ? null : shape.shape_id,
                )
              }
            ...
            >
              ...
              <span className="text-[10px] text-muted-2 truncate max-w-[80px]">
                {shape.layer}
              </span>
            </div>
          );
