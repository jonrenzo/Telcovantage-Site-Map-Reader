...
export const THEMES: Record<
  ColorTheme,
  { ok: string; review: string; corrected: string; label: string }
> = {
  default: {
    ok: "#16a34a",
    review: "#d97706",
...
export default function ReviewLayout({
  ...
  const handleCorrection = useCallback(
    (digitId: number, value: string | null) => {
      setResults((prev) =>
        prev.map((r) =>
          r.digit_id === digitId
  ...
  const confirmManual = useCallback(() => {
    ...
    if (ctx) {
      ...
      const minX = cx - PADDING,
        maxX = cx + PADDING;
      const minY = cy - PADDING,
        maxY = cy + PADDING;
    ...
    const newId =
      results.length > 0 ? Math.max(...results.map((r) => r.digit_id)) + 1 : 0;
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      {/* ── Sidebar ── */}
      <div
        className={`relative flex-shrink-0 transition-all duration-300 ease-in-out ${sidebarOpen ? "w-80" : "w-0"}`}
      >
        <div
      ...
      <div className="flex-1 relative overflow-hidden bg-[#e8edf5]">
        ...
        {selectedResult && !manualMode && (
          <DetailPanel
            ...
            onSave={(val) =>
              handleCorrection(selectedResult.digit_id, val || null)
            }
