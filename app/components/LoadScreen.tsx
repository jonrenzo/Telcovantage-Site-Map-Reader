  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (data.path.toLowerCase().endsWith(".pdf")) {
        ...
        setProgress(null);
        alert(
          "File could not be converted to DXF. Please upload a DXF directly.",
        );
        return;
      }
        ...
        <div className="flex">
          ...
          <div className="w-72 shrink-0 border-r border-border p-8 flex flex-col items-center justify-center gap-4 bg-[#f8f9fc]">
            ...
            >
              ...
              {uploadState === "loaded" ? (
                <>
                  <div className="font-semibold text-sm text-ok">
                    File loaded
                  </div>
                  <div className="text-ok text-xs font-semibold mt-1 break-all">
                    ✓ {displayName}
                  </div>
          ...
          <div className="flex-1 p-8 flex flex-col justify-center gap-5">
            <div>
              <h2 className="text-xl font-bold mb-1">Open Plan</h2>
